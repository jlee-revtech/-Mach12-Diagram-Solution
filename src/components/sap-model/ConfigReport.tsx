  ...
  return (
    <div className="bg-white rounded-lg border border-border shadow-card p-5">
      <div className="mb-3">
        ...
        {hint && <p className="text-body-sm text-text-secondary mt-0.5">{hint}</p>}
      </div>
      {children}
    </div>
  )
...
const th = 'text-left text-label uppercase text-text-secondary font-medium px-2.5 py-1.5'
const td = 'px-2.5 py-1.5 text-body-sm text-text-secondary border-t border-border'
    ...
    <div className="space-y-4 max-w-[1100px] pb-12">
      ...
      >
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2">
          {[
            ...
            return (
              <div key={c.label} className="rounded-lg border border-border px-2.5 py-2" style={{ background: meta.color + '0d' }}>
                <div className={`text-lg font-bold ${mono}`} style={{ color: meta.color }}>{c.value}</div>
                <div className="text-[10px] text-text-tertiary leading-tight">{c.label}</div>
              </div>
            )
          })}
        </div>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {m.companyCodes.map((c) => (
                <tr key={c.bukrs}>
                  <td className={`${td} ${mono} font-semibold text-text-primary`}>{c.bukrs}</td>
            ...
            <tbody>
              {m.assignments.map((a) => (
                <tr key={a.relationship}>
                  <td className={`${td} text-text-primary font-medium`}>{a.relationship}</td>
                  <td className={`${td} ${mono}`}>{a.via}</td>
                  <td className={`${td} ${mono} font-semibold`}>{a.count}</td>
                  <td className={`${td} text-text-tertiary`}>{a.note}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Section title="Plants & Storage Locations" hint="Plants tie to a company code through their valuation area (T001K); storage locations (T001L) are inventory-managing sub-locations within a plant.">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {m.plants.map((p) => (
            <div key={p.werks} className="flex items-start gap-2.5 rounded-lg border border-border px-2.5 py-2">
              <span className={`${mono} text-body-sm font-bold shrink-0`} style={{ color: ENTITY_META.plant.color }}>{p.werks}</span>
              <div className="min-w-0">
                <div className="text-body-sm text-text-secondary truncate">{p.name} <span className="text-text-tertiary">· CC {p.bukrs}</span></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Section title="Sales Organizations" hint="TVKO - one company code each.">
          <div className="space-y-1.5">
            {m.salesOrgs.map((s) => (
              <div key={s.vkorg} className="flex items-center gap-2 text-body-sm">
                ...
                <span className={`${mono} text-[10px] text-text-tertiary ml-auto`}>CC {s.bukrs}</span>
              </div>
            ))}
          </div>
        </Section>
        <Section title="Purchasing Organizations" hint="T024E (company code) + T024W (plant assignment).">
          <div className="space-y-1.5">
            {m.purchasingOrgs.map((p) => (
              <div key={p.ekorg} className="flex items-center gap-2 text-body-sm">
                ...
                <span className={`${mono} text-[10px] text-text-tertiary ml-auto`}>{p.plants.length ? `plants ${p.plants.join(', ')}` : `CC ${p.bukrs}`}</span>
              </div>
            ))}
          </div>
        </Section>
            ...
            <tbody>
              {m.raKeys.map((r) => (
                <tr key={r.key}>
                  ...
                  <td className={`${td} ${mono} text-text-tertiary`}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {m.raProjects.map((p) => (
                <tr key={p.project}>
                  <td className={`${td} ${mono} font-semibold text-text-primary`}>{p.project}</td>
                  ...
                  <td className={`${td} ${mono} text-text-tertiary`}>{p.keys.join(', ')}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Section title={`Profit Centers by Company Code - ${m.profitCenters.total} total`} hint="Profit centers live at controlling-area level but are aligned to company codes via CEPC_BUKRS. Here they are grouped within each company code.">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {m.companyCodes.filter((c) => (m.profitCentersByCompanyCode[c.bukrs] ?? []).length).map((c) => (
            <div key={c.bukrs} className="rounded-lg border border-border p-2.5">
              <div className="flex items-baseline gap-2 mb-1.5">
                <span className={`${mono} text-body-sm font-bold text-text-primary`}>{c.bukrs}</span>
                <span className="text-[10px] text-text-tertiary truncate">{c.name}</span>
                ...
                <div className="space-y-0.5">
                  {shown.map((x) => (
                    <div key={x.kostl} className="flex items-center gap-2 text-[10px]">
                      ...
                      <span className={`${mono} text-[9px] text-text-tertiary ml-auto shrink-0`}>{x.prctr}</span>
                    </div>
                  ))}
                  {list.length > shown.length && <div className="text-[9px] text-text-tertiary pt-0.5">+{list.length - shown.length} more</div>}
                </div>
              </div>
            )
          })}
