#!/usr/bin/env node
// Ingest scripts/skills/*.md as GLOBAL agent skills in the shared Knowledge
// Repository, tagged to EVERY workstream so every consultant agent (both apps)
// retrieves them. Use for cross-cutting training that applies to all streams
// (e.g. how to write functional/technical specs for enhancements).
//
// RE-RUNNABLE + EXTENSIBLE: drop a new <slug>.md in scripts/skills/ and re-run.
// The kb source code is `skill-<slug>`; the title is the first `# heading`.
// Idempotent (upsert by code + re-chunk + re-embed voyage-3).
...
async function main() {
  ...
  const files = existsSync(SKILLS_DIR) ? readdirSync(SKILLS_DIR).filter((f) => f.endsWith('.md')) : []
  if (!files.length) { console.log('No skill .md files in scripts/skills/.'); return }

  for (const file of files) {
    const body = readFileSync(join(SKILLS_DIR, file), 'utf8')
    const slug = basename(file, '.md')
    const code = `skill-${slug}`
    ...
    const row = {
      code, title, description: `Global agent skill: ${title}`.slice(0, 400),
      kind: 'skill', origin: 'diagram-app', tenant_key: null, workstream_codes: allCodes,
      version: null, frontmatter: { global: true, file }, body, source_app: 'mach12-diagram', updated_at: new Date().toISOString(),
    }
    let sourceId
  ...
  console.log(`\nDone. ${files.length} global skill(s) synced. Embeddings:`, VOYAGE_KEY ? 'voyage-3' : 'DISABLED (set VOYAGE_API_KEY)')
}
