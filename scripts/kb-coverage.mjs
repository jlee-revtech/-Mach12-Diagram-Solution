...
const GENRES = ['config', 'capModel', 'crosscut', 'L0-skill', 'L1-govcon', 'L2-industry', 'L3-dassian', 'L4-template', 'accelerator', 'criteria']
...
function verdict(c) {
  const domain = c.genres['L0-skill'].length + c.genres['L1-govcon'].length + c.genres['L2-industry'].length + c.genres['L3-dassian'].length
...
console.log('workstream'.padEnd(24) + ' xc cfg cap  L0  L1  L2  L3  tpl acc crit  verdict')
for (const w of WORKSTREAMS) {
  const c = cov[w.code]
  const g = c.genres
  const row =
    w.shortName.padEnd(24) +
    [cell(g.crosscut.length), cell(g.config.length), cell(g.capModel.length), cell(g['L0-skill'].length), cell(g['L1-govcon'].length), cell(g['L2-industry'].length), cell(g['L3-dassian'].length), cell(g['L4-template'].length), cell(g.accelerator.length), cell(g.criteria.length)].join(' ') +
    '   ' + verdict(c) + (w.isPlatform ? ' (platform)' : '')
  console.log(row)
}
console.log('\nlegend: xc=cross-cutting (all-stream)  cfg=config-capability  cap=capability-model')
console.log('        L0=stream SAP/technical skill  L1=GovCon overlay  L2=industry-model  L3=Dassian')
